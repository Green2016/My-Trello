...
var initBugFreeProject = function(json){
  var _arr = [];
  json.forEach((item)=>{
    var _item = {text:item.name};
    _arr.push(_item);
  });
  return _arr;
};
...
var boardButtonCallback = function(t){
  /*var _array = [];
  var url = "https://www.jirui.me:8089/api/";
  var _items = fetch(url).then(function(response){
    return response.json();
  }).then(function(data){
    _array= initBugFreeProject(data);
    console.log(_array);
  }).catch(function(err){
    console.log(err);
  });
